# FinanceOS Pro — Empieza aquí

Gracias por adquirir FinanceOS Pro.

**Empresa:** MAXNOVA & LUCI Global LLC  
**Soporte:** maxnovaluciglobal@gmail.com (60 días desde la compra)

---

## ¿Cómo usar FinanceOS Pro?

FinanceOS Pro tiene dos formas de uso:

---

## Opción 1 — Usar como app PWA (recomendada para empezar)

Usa FinanceOS Pro directamente como app instalable.
No necesitas hacer ningún deploy ni configuración técnica para empezar.

**App:**
https://financeos-hazel.vercel.app/app/

**En iPhone:**
1. Abre FinanceOS en Safari
2. Toca el botón Compartir
3. Selecciona "Agregar a pantalla de inicio"
4. Toca "Agregar"

**En Android:**
1. Abre FinanceOS en Chrome
2. Toca el menú de tres puntos
3. Selecciona "Agregar a pantalla principal" o "Instalar app"
4. Confirma

**En escritorio (Chrome o Edge):**
1. Abre FinanceOS en Chrome o Edge
2. Busca el ícono de instalación en la barra del navegador
3. Haz clic en "Instalar"

---

## Opción 2 — White-label / deploy propio (opcional, avanzado)

Si quieres entregar una experiencia con tu propia marca,
puedes personalizar FinanceOS y publicar tu propia versión.

Puedes ajustar:
- Nombre de marca
- Colores
- Moneda
- Textos principales
- Datos de contacto

**Para esta opción necesitas:**
- Node.js 18 o superior
- Conocimientos básicos de terminal
- Cuenta en Vercel, Netlify u otro servicio similar

Consulta los archivos incluidos:
- `03-WHITE-LABEL-GUIDE.md`
- `04-DEPLOY-TECHNICAL-GUIDE.md`

> **Nota:** Esta opción es completamente opcional. Puedes usar
> FinanceOS Pro como PWA sin publicar ninguna versión propia.

---

---

## Tus primeros 10 minutos

Sigue estos pasos para sacar el máximo provecho desde el inicio:

**1. Completa el onboarding (2 min)**
Al abrir por primera vez, elige tu moneda y perfil. FinanceOS carga categorías sugeridas según tu situación.

**2. Registra tu primer ingreso (1 min)**
Ve a Ingresos → "+ Nuevo ingreso". Agrega tu sueldo o entrada principal del mes.

**3. Registra 3 gastos del mes (2 min)**
Ve a Egresos → "+ Agregar egreso". Empieza con tus gastos más grandes: arriendo, supermercado, transporte.

**4. Agrega tus suscripciones (2 min)**
En Egresos → "Pago recurrente". Registra Netflix, Spotify, gimnasio. Verás cuánto suman al año.

**5. Crea un presupuesto (1 min)**
Ve a Presupuestos y define un límite para tu categoría más grande. La barra te muestra cuánto llevas gastado.

**6. Haz tu primer backup (2 min)**
Ve a Ajustes → "Crear respaldo". Guarda el archivo en un lugar seguro. **Este paso protege toda tu información.**

Listo. Ya tienes tu dashboard funcionando con datos reales.

---

## Lo que incluye tu plan

- Todo lo de FinanceOS Personal
- Instalación como app PWA
- Licencia comercial hasta 30 clientes
- Modo Asesor completo
- FinanceOS Coach integrado al Modo Asesor
- Reporte PDF profesional con suscripciones
- Revisión de suscripciones en Modo Asesor y PDF
- White-label opcional (config.js)
- Deploy propio opcional
- Guía de entrega al cliente
- Guía técnica de deploy
- Soporte técnico 60 días por email
- Actualizaciones 12 meses

---

## Modo Asesor

El Modo Asesor está en el sidebar de la app → sección "Asesor".

Incluye:
- Score financiero 0-100
- Semáforo verde/amarillo/rojo
- Alertas automáticas
- FinanceOS Coach integrado
- Revisión de gastos recurrentes estimados
- Notas del asesor
- Exportación de reporte PDF

---

## Importante — Backups

Cada cliente tiene su propia instancia independiente.
Los datos de cada cliente se guardan en su dispositivo.

Instruye a tus clientes para crear backups periódicos
desde **Ajustes → Crear respaldo**.

---

## Documentación completa

https://financeos-landing-omega.vercel.app/docs/

---

## Soporte

maxnovaluciglobal@gmail.com
Asunto: [FinanceOS Pro - Soporte]
Tiempo de respuesta: 48 horas hábiles

---

## Aviso legal

FinanceOS Pro es una herramienta de organización, seguimiento
y educación financiera. No constituye asesoría financiera, tributaria,
legal ni de inversión. No reemplaza el criterio profesional del asesor.

© 2026 MAXNOVA & LUCI Global LLC
