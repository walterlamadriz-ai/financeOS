...
import {
  parseCSV, detectColumns, validateRows, detectDuplicates,
...
const s = {
  ...
  dropIcon: { fontSize: 28, marginBottom: 10, color: 'var(--th)' },
  dropText: { fontSize: 14, fontWeight: 600, color: 'var(--tx)', marginBottom: 4 },
  dropSub: { fontSize: 12, color: 'var(--th)', fontFamily: 'var(--mono)' },
  ...
  select: {
    width: '100%', background: 'var(--sur2)', border: '.5px solid var(--brd2)',
    borderRadius: 6, padding: '7px 10px', color: 'var(--tx)', fontSize: 13,
    fontFamily: 'var(--mono)',
  },
  ...
  historyEmpty: { textAlign: 'center', padding: 32, color: 'var(--th)', fontSize: 13, fontFamily: 'var(--mono)' },
...
export default function ImportCSV() {
  ...
  function handleFile(f) {
    if (!f || !f.name.endsWith('.csv')) {
      setWarning('Solo se aceptan archivos .csv')
      return
    }
    setWarning(null)
  ...
  // ── STEP 2 → 3: Validar y detectar duplicados ─────────────────────────────
  async function handlePreview() {
    ...
    const withInclude = withDupes.map(r => ({ ...r, _include: r.status !== 'error' }))
    setRows(withInclude)
    setStep(2)
  }
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        <p style={s.sub}>// Sube un CSV de tu banco · procesado localmente · sin envío de datos</p>
      </div>
        ...
        <>
          <div style={s.card}>
            ...
            >
              ...
              <div style={s.dropText}>Arrastra el archivo aquí o haz clic para seleccionarlo</div>
              <div style={s.dropSub}>Solo archivos .csv · máximo {MAX_ROWS} filas</div>
            </div>
            <input id="csv-input" type="file" accept=".csv" style={{ display: 'none' }}
              onChange={e => e.target.files[0] && handleFile(e.target.files[0])} />
            <div style={s.privacy}>
              <span>◑</span>
              <span>El archivo se procesa localmente en tu navegador. FinanceOS no envía tus movimientos bancarios a servidores.</span>
            </div>
          </div>
            ...
            <div style={s.card}>
              ...
              <table style={s.table}>
                ...
                <tbody>{history.map((b, i) => (
                  <tr key={i}>
                    <td style={{ ...s.td, color: 'var(--tx)', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.fileName}</td>
                    ...
                    <td style={{ ...s.td, color: 'var(--th)' }}>{b.skippedRows + b.duplicateRows}</td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
          ...
          {history.length === 0 && (
            <div style={s.historyEmpty}>
              Aún no has importado movimientos. Descarga un CSV desde tu banco y súbelo aquí para ahorrar tiempo de registro manual.
            </div>
          )}
        </>
      )}
        ...
        <div style={s.card}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
              {[['single', 'Columna única de monto'], ['debit_credit', 'Columnas débito / crédito']].map(([val, lbl]) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
            {[
              ...
              { key: 'account', label: 'Cuenta (opcional)' },
            ].map(({ key, label }) => (
              <div key={key}>
                <div style={s.label}>{label}</div>
                <select style={s.select} value={mapping[key] || ''} onChange={e => setMapping(m => ({ ...m, [key]: e.target.value }))}>
      ...
      {step === 2 && (
        <>
          <div style={s.card}>
            ...
            <div style={{ fontSize: 11, color: 'var(--th)', fontFamily: 'var(--mono)', marginBottom: 12, padding: '8px 10px', background: 'var(--sur2)', borderRadius: 6 }}>
              Los duplicados se detectaron comparando fecha, descripción y monto con movimientos ya registrados en FinanceOS.
            </div>
            <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
              <button style={{ ...s.btn, ...s.btnSecondary, fontSize: 12 }}
                onClick={() => setRows(r => r.map(row => ({ ...row, _include: row.status === 'valid' })))}>
                Incluir solo válidos
              </button>
              <button style={{ ...s.btn, ...s.btnSecondary, fontSize: 12 }}
                onClick={() => setRows(r => r.map(row => ({ ...row, _include: true })))}>
                Incluir todos
              </button>
              <button style={{ ...s.btn, ...s.btnSecondary, fontSize: 12 }}
                onClick={() => setRows(r => r.map(row => ({ ...row, _include: false })))}>
                Deseleccionar todos
              </button>
            </div>
          </div>
                ...
                <tr key={i} style={{ opacity: row._include ? 1 : .45 }}>
                  <td style={s.td}>
                    <input type="checkbox" checked={!!row._include}
                      onChange={e => setRows(r => r.map((x, j) => j === i ? { ...x, _include: e.target.checked } : x))} />
                  </td>
                  ...
                  <td style={{ ...s.td, color: row.type === 'income' ? 'var(--accent)' : 'var(--red)', fontWeight: 600 }}>
                    {row.type === 'income' ? '+' : '−'}{sym}{fmt(row.amount)}
                  </td>
                  <td style={{ ...s.td, color: 'var(--th)' }}>{row.type === 'income' ? 'Ingreso' : 'Gasto'}</td>
                  <td style={s.td}>
                    <span style={s.badge(row.status)}>
          ...
          <div style={s.disclaimer}>
            Esta función ayuda a importar y organizar movimientos. No constituye asesoría financiera, tributaria, legal ni de inversión.
          </div>
        </>
      )}
        ...
        <div style={s.card}>
          {result?.demo ? (
            <div style={{ textAlign: 'center', padding: 24 }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--th)', fontFamily: 'var(--mono)' }}>En el modo demo no se guardan datos reales. Compra FinanceOS para importar movimientos reales.</p>
            </div>
          ) : (
            <div style={{ textAlign: 'center', padding: 24 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, margin: '20px 0', maxWidth: 400, marginLeft: 'auto', marginRight: 'auto' }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--th)', fontFamily: 'var(--mono)', marginBottom: 16 }}>
                Los movimientos ya aparecen en Dashboard, Ingresos y Gastos. FinanceOS Coach puede generar nuevas señales.
              </p>
